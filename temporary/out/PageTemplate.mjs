import { JSMethodTemplate, JSPageTemplate, TSMethodTemplate, TSPageTemplate } from './Templates.mjs';
...
export default class PageTemplate extends RootTemplate {
    ...
    _generateMethods(methods, template) {
        return methods
            .map(method => this._replacePlaceholders(template, method))
            .join("\n");
    }
    _replacePlaceholders(template, placeholders) {
        return Object.keys(placeholders).reduce((updatedTemplate, key) => updatedTemplate.replace(new RegExp(`{{${key}}}`, "g"), placeholders[key]), template);
    }
