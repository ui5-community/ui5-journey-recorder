...
import { PageBuilder } from "./PageBuilder.class";
...
export default class ViewPageBuilder extends PageBuilder {
    ...
    private _createBaseSetup(step: Step) {
        this._viewName = step.viewInfos?.relativeViewName || this._viewName;
        const posNamespace = step.viewInfos?.absoluteViewName
            .replace(`.${step.viewInfos?.relativeViewName}`, '')
            .replace('.view', '')
            .trim();
    ...
    private _generateDependencies(): StringBuilder {
        const dependencies = new StringBuilder('sap.ui.define([').addNewLine();
